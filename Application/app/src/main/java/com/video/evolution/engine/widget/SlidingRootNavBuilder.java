...
import com.video.evolution.engine.widget.callback.DragListener;
import com.video.evolution.engine.widget.callback.DragStateListener;
import com.video.evolution.engine.widget.transform.CompositeTransformation;
import com.video.evolution.engine.widget.transform.ElevationTransformation;
import com.video.evolution.engine.widget.transform.RootTransformation;
import com.video.evolution.engine.widget.transform.ScaleTransformation;
import com.video.evolution.engine.widget.transform.YTranslationTransformation;
import com.video.evolution.engine.widget.util.ActionBarToggleAdapter;
import com.video.evolution.engine.widget.util.DrawerListenerAdapter;
import com.video.evolution.engine.widget.util.HiddenMenuClickConsumer;
...
public class SlidingRootNavBuilder {
    ...
    private ViewGroup getContentView() {
        ...
        if (contentView.getChildCount() != 1) {
            throw new IllegalStateException(activity.getString(R.string.evolution_ex_bad_content_view));
        }
        return contentView;
    }
    ...
    private View getMenuViewFor(SlidingRootNavLayout parent) {
        if (menuView == null) {
            if (menuLayoutRes == 0) {
                throw new IllegalStateException(activity.getString(R.string.evolution_ex_no_menu_view));
            }
    ...
    protected void initToolbarMenuVisibilityToggle(final SlidingRootNavLayout sideNav, View drawer) {
        if (toolbar != null) {
            ...
            ActionBarDrawerToggle toggle = new ActionBarDrawerToggle(activity, dlAdapter, toolbar,
                    R.string.evolution_drawer_open,
                    R.string.evolution_drawer_close);
            toggle.syncState();
